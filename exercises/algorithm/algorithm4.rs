    ...
    T: Ord,
{
    ...
    fn insert(&mut self, value: T) {
        match value.cmp(&self.value) {
            Ordering::Equal => (), // Don't insert duplicate values
    ...
    fn search(&self, value: T) -> bool {
        match value.cmp(&self.value) {
            Ordering::Equal => true,
            Ordering::Less => self.left.as_ref().is_some_and(|node| node.search(value)),
            Ordering::Greater => self.right.as_ref().is_some_and(|node| node.search(value)),
        }
    }
}
